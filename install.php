...
// A/B Test Events Tabelle erstellen
$table = rex_sql_table::get(rex::getTable('ab_test_events'));
$table
    ->ensurePrimaryIdColumn()
    ->ensureColumn(new rex_sql_column('test_id', 'int(11)'))
    ->ensureColumn(new rex_sql_column('variant', 'varchar(10)'))
    ->ensureColumn(new rex_sql_column('event', 'varchar(50)', false, 'view'))
    ->ensureColumn(new rex_sql_column('details', 'text'))
    ->ensureColumn(new rex_sql_column('click_url', 'varchar(500)'))
    ->ensureColumn(new rex_sql_column('click_text', 'varchar(255)'))
    ->ensureColumn(new rex_sql_column('ip_hash', 'varchar(255)'))
    ->ensureColumn(new rex_sql_column('user_agent', 'varchar(255)'))
    ->ensureColumn(new rex_sql_column('session_id', 'varchar(255)'))
    ->ensureColumn(new rex_sql_column('created_at', 'datetime'))
    ->ensureIndex(new rex_sql_index('idx_test', ['test_id', 'variant', 'event']))
    ->ensureIndex(new rex_sql_index('idx_date', ['created_at']))
    ->ensureIndex(new rex_sql_index('idx_session', ['session_id']))
    ->ensureIndex(new rex_sql_index('idx_click', ['event', 'click_url']))
    ->ensure();
...
// rex_article Tabelle erweitern (wie YRewrite)
$table = rex_sql_table::get(rex::getTable('article'));
$table
    ->ensureColumn(new rex_sql_column('art_ab_variant', 'int(11)', true))
    ->alter();

// Historie aus aktuellen Konfigurationen befüllen
$sql = rex_sql::factory();
$sql->setQuery('
...
echo rex_view::success('A/B Tests Addon wurde erfolgreich installiert.');
